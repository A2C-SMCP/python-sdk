...
模块组成 / Module composition:
  ...
  - thresholds.py inline budget / too_large cap / chunk max / upload 会话阈值 默认值 + 环境变量覆盖
  - upload.py     ``client:put_blob`` 上行写入的有界上传会话（v0.4.0：``.part`` + 增量 sha256 + 原子 rename）
...
from a2c_smcp.computer.blob.upload import (
    BlobUploadStore,
    sanitize_name_hint,
)
